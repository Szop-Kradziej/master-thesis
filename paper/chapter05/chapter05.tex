...
\label{run-platform}
TODO: what with IP address for cors? also in system envs?

W celu poprawnego uwierzytelnienia użytkowników przez GitHub należy zarejestrować aplikację w serwisie.
...
Następnie należy utworzyć schemat bazy danych.
Można to zrobić bezpośrednio instalując na maszynie PostgreSQL lub przy użyciu kontenera dockerowego.
Przykładowa komenda do uruchomienia kontenera z bazą wygląda następująco:
...
Po uruchomieniu bazy należy utworzyć tabele.
W przypadku kontenera dockerowego można użyć do tego poniższej komendy, gdzie \textit{platform\_schema.sql} jest plikiem zawierającym definicję schematu bazodanowego, który został załączony do pracy:
...
Aby uruchomić serwer należy przejść do katalogu \textit{master\_thesis/test\_platform\_runner} i użyć komendy:
